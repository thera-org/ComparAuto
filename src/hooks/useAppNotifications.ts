...
export const useAppNotifications = () => {
  ...
  return {
    ...
    auth: {
      ...
      logoutSuccess: () => info('Logout realizado', 'Até logo!'),

      emailConfirmed: () => success('Email confirmado!', 'Sua conta foi ativada com sucesso.'),
    ...
    crud: {
      ...
      saveSuccess: () => success('Salvo com sucesso!', 'Suas alterações foram salvas.'),
    ...
    upload: {
      ...
      uploadProgress: (fileName: string) => info('Enviando arquivo...', `Enviando ${fileName}...`),
    ...
    validation: {
      ...
      invalidEmail: () => warning('Email inválido', 'Digite um endereço de email válido.'),

      passwordMismatch: () =>
        warning('Senhas não coincidem', 'As senhas digitadas não são iguais.'),

      weakPassword: () => warning('Senha fraca', 'A senha deve ter pelo menos 8 caracteres.'),
    ...
    system: {
      ...
      sessionExpired: () => warning('Sessão expirada', 'Faça login novamente para continuar.'),

      permissionDenied: () =>
        error('Acesso negado', 'Você não tem permissão para realizar esta ação.'),

      loadingComplete: () => info('Carregamento concluído', 'Dados carregados com sucesso.'),
    },
